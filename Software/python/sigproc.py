...
class SigProc(object):
    soname = os.getenv('TMS_HOME_DIR')+"/python/build/sigproc.so"
    sigprocSO = cdll.LoadLibrary(soname)
    ...
    @classmethod
    def filters_trapezoidal(cls, nSamples, adcChData, scrAry, fltParam=[500, 150, 200, -1.0]):
        '''Have not make this work yet'''
        cfun = cls.sigprocSO.filters_trapezoidal
        cfun(c_size_t(nSamples), adcChData, scrAry, c_size_t(fltParam[1]), c_size_t(fltParam[2]), c_double(fltParam[3]));
