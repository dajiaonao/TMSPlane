...
class bkgEstimator:
    def __init__(self, cfile=None):
        ...
        if cfile: self.get_data(cfile, 0)
        print self.A, self.phase

    def get_data(self, cfile='data/fpgaLin/Feb09b_data_1881.root',  evt=150):
        ch = TChain('tree1')
        ch.Add(cfile)
...
#         for i in range(len(sp1.scrArys)):
#             print(i, sp1.scrArys[i])

#         vx = np.array([sp1.scrArys[ich][i] for i in range(sp1.nSamples)])
        vx = np.array([sp1.scrAry[i] for i in range(sp1.nSamples)])
        vo = va
#         vo = data1[ich*sp1.nSamples:(ich+1)*sp1.nSamples]
